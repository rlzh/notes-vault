...
- **Action**: attacker chooses a system configuration to attack at beginning of each time step 
	- Denoted as $\tilde{s}^{t}$ 
	- Attack fails if attacker chooses wrong system configuration; $\tilde{s}^t\neq s^t$ 
	- Attack succeeds with some probability $\mu_{s^t}$ (**attack success rate**); otherwise, and fails with probability $1-\mu_{s^t}$ 
	- Successful attack lead to a loss for the defender
		- Loss is based on [[CVSS]] impact score
...
- **Proposed solution:**
	- Train a meta-policy on a variety of attacks in simulated environment (using attack types for existing databases and penetration testing tools)
...
- Assume defender commits to a stationary policy $\pi_\mathcal{D}$
	- ***Response function***: the attacker chooses a policy $\textbf{q}_s \in R(s,\pi_\mathcal{D})$  for any state $s$, where $R(\cdot, \pi_\mathcal{D})$ denotes a **function that** **provides** **a set of response policies for the attacker**
...
- **Proof:** consider the following MDP for the defender $(S, A', \mathcal{P}', r', \gamma)$ which is derived from the earlier [[#Markov Game Model for MTD]]
	...
	- $A' = \Delta(S)$ is the action space of the defender
		- Redefine the defender's action in state $s$ as the **probability vector (i.e., policy)** $\textbf{p}_s$
	...
	- $r': S \times A' \rightarrow \mathbb{R}_{\leq 0}$ is the reward function for the defender
		...
		- The **defender's reward function** can be defined as
		$$r'(s^{t-1}, \textbf{p}_{s^{t-1}}) = \sum_{s^t} p_{s^{t-1}s^t} q_{s^{t-1}\tilde{s}^t}r_\mathcal{D}(s^{t-1}, a^t)$$ 
...
- View defender's problem of solving its MDP against a particular attack denoted, $\xi^k=(\mu_k,\textbf{l}_k, R_k)$ 
	...
	- $\alpha$ as step size during gradient descent when training meta policy
